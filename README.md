...
__Метод потенциальных функций__ - метрический классификатор. В отличии от метода парзеновского окна, окружность строится вокруг обучающих точек.
При классификации объект проверяется на близость к объектам из обучающей выборки. Можно сказать, что объекты из обучающей выборки «заряжены» своим классом, а вес каждого из них при классификации зависит от «заряда» и расстояния до классифицируемого объекта.
...
Алгоритм метода PF
   ...
   - Суммируем веса объектов одинаковых классов. Доминириющий класс присваивается точке.
