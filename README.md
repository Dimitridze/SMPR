...
Удаляем из выборки все выбросы (объекты, отступ которых меньше порога фильтрации выбросов).Затем пересчитав все отступы заново, взять по одному эталону из каждого класса (объекты с наибольшим положительным отступом), и добавить их в множество эталонов.
Классифицируем объекты обучающей выборки, с XL =  множеству эталонов. Считаем число ошибок.
