...
Алгоритм метода PF
   ...
   - Изначально для каждого объекта выборки задаётся *ширина окна* ![h_i](http://latex.codecogs.com/gif.latex?h_%7Bi%7D) эмпирически (выбирается из собственных соображений).
   - Затем для каждого объекта выборки задаётся *сила потенциала* ![gamma_i](http://latex.codecogs.com/gif.latex?%5Cgamma_%7Bi%7D). 
   ...
   - Суммируются веса объектов одинаковых классов. Класс с самым большим весом присваивается точке.
...
Алгоритм подбора ![gamma_i](http://latex.codecogs.com/gif.latex?%5Cgamma_%7Bi%7D):
   ...
   - Если же полученный класс не совпал с реальным, то *сила потенциала* для выбранного объекта увеличивается на 1. Иначе снова выбирается объект и классифицируется.
...
__Код алгоритма подбора ![gamma_i](http://latex.codecogs.com/gif.latex?%5Cgamma_%7Bi%7D):__
...
В данной программе использовал прямоугольное ядро. Алгоритм подбирает только силу потенциала , радиус потенциалов h задаются эмпирически().
Задать значение ширину окна для каждого объекта
  
     SeparateH <- function(xl) 
     {
